 ...
 */
export async function convertToHLS(inputPath, outputDir = path.join('tmp', 'hls')) {
 ...
 * @returns Promise with output m3u8 file path
 */
export async function convertToHLSWithTranscode(inputPath, outputDir = path.join('tmp', 'hls'), quality = 'medium') {
    await fs.mkdir(outputDir, { recursive: true });
    const videoId = path.basename(inputPath, path.extname(inputPath));
    const hlsDir = path.join(outputDir, videoId);
    await fs.mkdir(hlsDir, { recursive: true });
 ...
 * @returns Promise with output m3u8 file path
 */
export async function downloadAndConvertToHLS(videoUrl, videoId, hlsDir = path.join('tmp', 'hls')) {
